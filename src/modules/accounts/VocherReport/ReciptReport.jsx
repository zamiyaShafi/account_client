...
export default function ReciptReport() {
  ...
  const selectedVoucherType = 'Receipt'; // Set to Receipt for receipt report
  ...
  useEffect(() => {
    if (selectedVoucherType === 'Receipt') {
      const filteredDebit = accounts.filter((account) =>
        ['Cash-in-Hand', 'Bank Accounts'].includes(account?.group_id?.groupName)
      );
      setFilteredDebitAccounts(filteredDebit);

      const filteredCredit = accounts.filter((account) =>
        ['Sundry Debtors', 'Current Assets', 'Income'].includes(account?.group_id?.groupName)
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const filterParams = {
        ...
        amount: data.amount,
        voucher_type: "Receipt", // Set to Receipt
      };
      ...
      if (fetchFilteredTransactions.fulfilled.match(resultAction)) {
        ...
        setTransactions(resultAction.payload);
      } else {
      ...
      <CRow>
        <CCol xs={12}>
          <CCard className="mb-4">
            <CCardHeader>
              <strong>Receipt</strong> <small>Report</small>
            </CCardHeader>
            <CCardBody>
            ...
            <CCardBody style={{ display: 'flex' }}>
              <CTable style={{ border: '2px solid black' }}>
                ...
                <CTableBody>
                  {transactions.filter(transaction => transaction.voucher_type === "Receipt").length > 0 ? (
                    transactions
                      .filter(transaction => transaction.voucher_type === "Receipt") // Filter transactions by voucher_type
                      .map((transaction, index) => (
                        <CTableRow key={index}>
                          ...
                          <CTableDataCell>
                            <CButton color="danger" onClick={() => handleDelete(transaction._id)}>Delete</CButton>
                          </CTableDataCell>
                        </CTableRow>
                      ))
                  ) : (
                    <CTableRow>
                      <CTableDataCell colSpan="8" className="text-center">No transactions found</CTableDataCell>
                    </CTableRow>
                  )}
                </CTableBody>
              </CTable>
