...
import { fetchLedgers, fetchFilteredTransactions } from '../../../store/ledgerSlice'; 
...
export default function JournalReport() {
  ...
  const selectedVoucherType = 'Journal';
  ...
  useEffect(() => {
    if (selectedVoucherType === 'Journal') {
      const filteredDebit = accounts.filter((account) =>
        ['Journal Accounts', 'Expenses (Direct)', 'Expenses (Indirect)', 'Direct Expenses', 'Fixed Assets', 'Indirect Expenses'].includes(account?.group_id?.groupName)
      );
      ...
      <CRow>
        <CCol xs={12}>
          <CCard className="mb-4">
            <CCardHeader>
              <strong>Journal</strong> <small>Report</small>
            </CCardHeader>
            <CCardBody>
            ...
            <CCardBody style={{ display: 'flex' }}>
              <CTable style={{ border: '2px solid black' }}>
                ...
                <CTableBody>
                  ...
                  {transactions.filter(transaction => transaction.voucher_type === "Journal").length > 0 ? (
    transactions
      .filter(transaction => transaction.voucher_type === "Journal") // Filter transactions by voucher_type "Journal"
